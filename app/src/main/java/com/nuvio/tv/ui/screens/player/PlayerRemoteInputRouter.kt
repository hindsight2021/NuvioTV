...
/**
 * Translates raw Android TV key events into high-level player actions while
 * maintaining state for held-key scrubbing, one-shot debouncing, and focus.
 *
 * During a held-key seek, repeated ACTION_DOWN events emit [PlayerRemoteAction.PreviewSeek]
 * so the UI can preview the destination timestamp before committing a
